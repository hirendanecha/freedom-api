...
exports.orderAutoMail = async (items) => {
  ...
  const sellerObject = items.reduce((prev, current) => {
    buyerArray.push({
      ...
      buyerName: current.buyer.user_first_name + " " + current.buyer.user_last_name,
    ...
    const objectToAdd = {
      ...
      sellerName: current.user_first_name + " " + current.user_last_name,
      buyerName: current.buyer.user_first_name + " " + current.buyer.user_last_name,
      unitPrice: current.sell_currency + current.sell_unit_price + " per " + current.product_unit,
  ...
  mailsArray.push(
    {
      ...
      templateData: { name: buyerArray[0].buyerName, totalAmount: `${buyerArray[0].sell_currency}${totalAmount}`, orders: buyerArray },
    })
    ...
    mailsArray.push(
      {
        ...
        templateData: { name: sellerObject[key][0].sellerName, finalAmount: `${sellerObject[key][0].sell_currency}${finalAmount}`, orders: sellerObject[key] },
      })
  })
...
exports.registrationMail = async (userData, userId) => {
  ...
  }, jwtSecretKey, { expiresIn: "730 days" });
  ...
  const mailObj = {
    email: userData.UserName,
    subject: "https://localfoods.market/register",
...
if(user.length){
  ...
  const token = jwt.sign({
    userId: user[0]?.Id
  }, jwtSecretKey, { expiresIn: environment.JWT_EXPIRED });

  let forgotPasswordUrl = `${environment.FRONTEND_URL}/set-password/user?accesstoken=${token}`;
  const mailObj = {
