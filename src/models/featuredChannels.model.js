...
featuredChannels.findChannelById = async function (id) {
  const query1 =
    "select f.*,p.Username,u.Username,u.Email,count(ca.profileId) as Admins from featured_channels as f left join profile as p on p.ID = f.profileId left join users as u on u.Id = p.UserID left join channelAdmins as ca on ca.channelId = f.id where f.id=?;";
  const query2 =
