...
class UnsubscribeProfile {
  ...
  static async getByProfileId(profileId) {
    return await executeQuery(
      `SELECT unsub_pr.Id, pr.ProfilePicName, pr.Username, pr.FirstName from unsubscribe_profiles as unsub_pr left join profile as pr on unsub_pr.unsubscribeProfileId = pr.ID where unsub_pr.profileId = ?`,
      [profileId]
    ) || [];
  }
}
